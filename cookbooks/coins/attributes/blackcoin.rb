# -*- encoding : utf-8 -*-
default[:coins][:blackcoin][:source] = 'https://github.com/rat4/blackcoin/archive/v1.0.5.1.tar.gz'
default[:coins][:blackcoin][:executable] = 'blackcoind'
default[:coins][:blackcoin][:rpc_user] = 'blackcoin'
default[:coins][:blackcoin][:rpc_pass] = '9aPXHYaWCqQ54FYXMBuKVsxQd2JoGgq7HunxDTt9mmQX'
default[:coins][:blackcoin][:rpc_port] = '15715'
default[:coins][:blackcoin][:rpc_allow_net] = '127.0.0.1'
default[:coins][:blackcoin][:wallet_location] = 'S3'
default[:coins][:blackcoin][:wallet_s3_bucket] = ''
default[:coins][:blackcoin][:wallet_s3_key] = ''
default[:coins][:blackcoin][:wallet_s3_secret] = ''

